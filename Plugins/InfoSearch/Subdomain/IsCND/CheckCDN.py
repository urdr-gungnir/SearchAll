...
logger = logger_object.SetModuleName("CheckCDN")
...
def checkCDN(subdomains_queue, notCDNSubdomains, CDNSubdomainsDict):
    while not subdomains_queue.empty():
        ...
        if not cnameRet:
            ipASNSRet, ips = ipASNSCheckCDN(subdomain)
            if not ipASNSRet:
                notCDNSubdomains.append((subdomain,ips))
                CDNSubdomainsDict[subdomain] = 'NOT'
            else:
                logger.info('{}: {}'.format(subdomain, ipASNSRet))
                # notCDNSubdomains.append(subdomain)
                CDNSubdomainsDict[subdomain] = ipASNSRet
        else:
            logger.info('{}: {}'.format(subdomain, cnameRet))
            # notCDNSubdomains.append(subdomain)
